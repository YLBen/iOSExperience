...
#define itemWidthHeight ((SCREEN_WIDTH-30)/2)
...
-(void)setupUI{
    ...
    self.collectionView.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT - kTopHeight - kTabBarHeight);
...
-(CGFloat)waterFlowLayout:(WaterFlowLayout *)WaterFlowLayout heightForWidth:(CGFloat)width andIndexPath:(NSIndexPath *)indexPath{
    ...
    if (personModel.hobbys && personModel.hobbysHeight == 0) {
        //计算hobby的高度 并缓存
        CGFloat hobbyH=[personModel.hobbys heightForFont:[UIFont systemFontOfSize:12.0f] width:(SCREEN_WIDTH-30)/2-20];
        if (hobbyH>43) {
            hobbyH=43;
        }
